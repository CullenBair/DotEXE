...
public class SellButtonScript : MonoBehaviour
{
    ...
    public void PressSellButton()
    {
        try
        {
			...
			InfoScript.instance().Displayer("You're trying to sell " + gm.GetComponent<GameManagerScript>().GetTile(lastTileIndex).GetComponent<TileScript>().GetName());
		}
        catch (Exception e)
        {
            Debug.Log("No tile is selected!");
			InfoScript.instance().Displayer("No tile is selected!");
        }
    }
}
